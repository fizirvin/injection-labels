...
class Labels extends Component {
  ...
  renderAmealcoLabels = () => {
    return this.state.plastics.map(
      ({ _id, header, color, text, intRef, pieces, machine }, index) => {
        const ins = this.props.setAmealcoInspector.find(
          (item) => item._id === _id && item.inspector
        );
        const inspector = ins ? ins.inspector : "";
        const op = this.props.setAmealcoOperator.find(
          (item) => item._id === _id && item.operator
        );
  ...
  renderVariasLabels = () => {
    return this.state.plastics.map(
      ({ _id, header, color, text, intRef, pieces, machine }, index) => {
        const ins = this.props.setVariasInspector.find(
          (item) => item._id === _id && item.inspector
        );
        const inspector = ins ? ins.inspector : "";
        const op = this.props.setVariasOperator.find(
          (item) => item._id === _id && item.operator
        );
        const operator = op ? op.operator : "";
  ...
  onClean = (e) => {
    ...
    if (this.props.team === "varias") {
      ...
      const qu = this.props.setVariasQuantity.find(
        (item) => item._id === _id && item.quantity
      );
      if (ins || op || qu) {
        return this.props.cleanVarias(_id);
      } else {
        return console.log("nada que resetear");
      }
    } else {
      const ins = this.props.setAmealcoInspector.find(
        (item) => item._id === _id && item.inspector
      ...
      const qu = this.props.setAmealcoQuantity.find(
        (item) => item._id === _id && item.quantity
      );
      if (ins || op || qu) {
  ...
  renderButton = () => {
    if (this.props.team === "varias") {
      return (
        <div>
          <button name="varias" onClick={this.onTeam} className="red">
            R Varias
          </button>
          <button name="amealco" onClick={this.onTeam}>
            Amealco
          </button>
        </div>
      );
      ...
      return (
        <div>
          <button name="varias" onClick={this.onTeam}>
            R Varias
          </button>
          <button name="amealco" onClick={this.onTeam} className="red">
            Amealco
          </button>
        </div>
      );
    ...
    return (
      <div>
        <div className="controls">
          ...
          {this.renderButton()}
          <div>
            <button onClick={this.saveConfig} disabled={this.state.button}>
              Save configuration
            </button>
            <button
          ...
          <div>
            <button onClick={this.onClean} disabled={this.state.edit}>
              Clean One
            </button>
          ...
          <div>
            ...
            <Link to={`/label/edit/${this.state.checked}`}>
              <button disabled={this.state.edit}>Edit</button>
            </Link>
          </div>
        </div>
