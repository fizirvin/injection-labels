...
class RowLabel extends Component {
  ...
  renderRowLabel = ({index, _id, header, color, text, intRef }) =>{
    return (
      <tr key={_id} className='row_label_selected'>
        ...
        <td className='table_body_row'><input type='text' size="12" maxLength='12' name={'lot'} className='input_lot' onChange={this.onInputChange} value={this.state.lot}></input></td>
  ...
  renderView = () =>{
    ...
    if(quantity > 32 || quantity < 1 || start < 1 || !quantity || !start || !pieces || !lot || !machine){
      return true
    }
    else{
      if(this.state.header === 'GEN AL5' || this.state.header === 'GEN AL6')
